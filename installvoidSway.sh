...
case "$installChoice" in
  ...
  y|Y|yes|Yes|YES )
    ...
    ler \
        ...
        xbps-install -Sv sway dbus elogind polkit xorg-server-xwayland

        xbps-install -Sv foot btop gvfs qutebrowser ranger Waybar wlogout grimshot neovim xorg-fonts fuzzel imv mpv wl-clipboard

        sudo reboot

        echo "Installing additional applications..."
        sudo xbps-install -S neofetch btop grub-customizer gvfs-mtp gvfs ntfs-3g mpv firefox wget git xz unzip zip nano file-roller gptfdisk xtools mtools mlocate 
